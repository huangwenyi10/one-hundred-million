...
> **⚠️ 目录唯一事实来源（2026-09-03 固化）**：本地 `目录/` 只有**一处权威位置**——自动化
> 生产主工作区 `/Users/ay/WorkBuddy/2026-08-30-21-04-18/目录/`（`update_catalog.py` 与
> cron 定时任务都在这跑）。**任何其它会话语境（当前会话工作区 / 新开的窗口）都不要另建
> `目录/`**——遇到「补录目录 / 同步目录」需求，一律读写上面这个主工作区的 `目录/`，再从它
> 同步到 tdrive。历史教训：曾出现「旧工作区 6 类 / 当前会话 3 类 / tdrive 3 类」三处分叉，
> 根因就是不同会话语境各自建了一份 `目录/`。同步前先 `sync_catalog_tdrive.py check
> --workspace /Users/ay/WorkBuddy/2026-08-30-21-04-18` 核对以主工作区为准。
...
| 写本地 `目录/`（**主工作区**，唯一事实来源，见顶部 ⚠️） | 读本地 → 上传/覆盖 tdrive `短视频目录/` |
